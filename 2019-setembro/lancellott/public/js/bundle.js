...
var app = (function () {
    ...
    class BigButton extends SvelteComponentDev {
    	constructor(options) {
    		...
    		if (ctx.action === undefined && !('action' in props)) {
    			console_1$1.warn("<BigButton> was created without expected prop 'action'");
    		}
    	}
    ...
    class FunctionalButton extends SvelteComponentDev {
    	constructor(options) {
    		...
    		if (ctx.action === undefined && !('action' in props)) {
    			console.warn("<FunctionalButton> was created without expected prop 'action'");
    		}
    		if (ctx.status === undefined && !('status' in props)) {
    			console.warn("<FunctionalButton> was created without expected prop 'status'");
    		}
    	}
    ...
    class MiddleButton extends SvelteComponentDev {
    	constructor(options) {
    		...
    		if (ctx.action === undefined && !('action' in props)) {
    			console.warn("<MiddleButton> was created without expected prop 'action'");
    		}
    		if (ctx.status === undefined && !('status' in props)) {
    			console.warn("<MiddleButton> was created without expected prop 'status'");
    		}
    	}
    ...
    class YellowButton extends SvelteComponentDev {
    	constructor(options) {
    		...
    		if (ctx.status === undefined && !('status' in props)) {
    			console.warn("<YellowButton> was created without expected prop 'status'");
    		}
    		if (ctx.action === undefined && !('action' in props)) {
    			console.warn("<YellowButton> was created without expected prop 'action'");
    		}
    	}
    ...
    class Keyboard extends SvelteComponentDev {
    	constructor(options) {
    		...
    		if (ctx.easterEgg === undefined && !('easterEgg' in props)) {
    			console.warn("<Keyboard> was created without expected prop 'easterEgg'");
    		}
    	}
    ...
    function instance$6($$self, $$props, $$invalidate) {
      ...
      // });
    	...
    	Object.keys($$props).forEach(key => {
    		if (!writable_props.includes(key) && !key.startsWith('$$')) console_1$2.warn(`<App> was created with unknown prop '${key}'`);
    	});
